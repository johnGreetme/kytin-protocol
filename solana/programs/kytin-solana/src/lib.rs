// 🦞 Kytin Protocol (Patent Pending)
// The decentralized hardware root-of-trust for Sovereign AI Agents.
// Optimized for Solana Alpenglow (SIMD-0266 Ready).
...
pub mod kytin_solana {
    ...
    pub fn sign_heartbeat(ctx: Context<SignHeartbeat>) -> Result<()> {
        ...
        #[cfg(not(feature = "mock-tpm"))]
        {
            // PROD: Verify actual TPM signature here in V2
            // For now, authority_tpm MUST sign this transaction
        }

        #[cfg(feature = "mock-tpm")]
        {
            msg!("⚠️ MOCK TPM LOGIC ENABLED: Bypassing hardware signature verification.");
        }
